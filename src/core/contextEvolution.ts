...
import { appendSoulEntries, readSoulText, type SoulEntry, type SoulSection } from './contextFiles.js';
...
type ContextEvolutionResult = {
  soul: SoulEntry[];
};

type ContextEvolutionGateResult = {
...
export function shouldConsiderSoulSync(messages: ChatMessage[]): boolean {
...
function parseGateResult(raw: string): ContextEvolutionGateResult {
  try {
    const parsed = JSON.parse(extractJsonObject(raw)) as { should_analyze?: boolean };
...
function buildGatePrompt(messages: ChatMessage[], soulText: string): string {
...
export async function shouldAnalyzeContextEvolution(
  messages: ChatMessage[],
  config: Config,
  signal?: AbortSignal,
): Promise<boolean> {
