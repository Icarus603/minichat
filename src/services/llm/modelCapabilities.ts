...
function isLikelyOpenRouterChatModel(id: string): boolean {
  ...
  return isLikelyApiChatModel(id.slice('openai/'.length));
}
...
function withCurrentModel(options: ModelOption[], currentModel: string): ModelOption[] {
  ...
  return [{ id: currentModel, description: 'Current model.' }, ...options];
}
...
export function getReasoningEffortOptions(config: StoredConfig, modelId: string): ReasoningEffort[] {
...
export function supportsReasoningEffort(config: StoredConfig, modelId: string): boolean {
  return getReasoningEffortOptions(config, modelId).length > 0;
}

export async function listAvailableModels(config: StoredConfig): Promise<ModelOption[]> {
  if (!config.apiKey) {
