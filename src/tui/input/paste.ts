...
export const isLargePaste = (text: string) =>
  text.split('\n').length > LARGE_PASTE_LINE_THRESHOLD || text.length > LARGE_PASTE_CHAR_THRESHOLD;

export const createPastePlaceholder = (text: string, existing: Record<string, string>) => {
...
export const expandPastePlaceholders = (text: string, pastedContent: Record<string, string>) => {
  let expanded = text;
