...
export const App = ({ sessionId, initialTranscript = [], onAuthAction }) => {
    ...
    const handleSend = async (input) => {
        ...
        try {
            ...
            const final = [...updated, ...statusMessages, { role: 'ai', content: response }];
            setTranscript(final);
            ...
            const final = [...updated, { role: 'ai', content: `Error: ${err instanceof Error ? err.message : String(err)}` }];
            setTranscript(final);
            saveTranscript(currentSessionId, final);
        }
        finally {
    ...
    const handleCommand = (cmd) => {
        ...
        if (cmd === '/login' || cmd === '/logout')
            return void onAuthAction(cmd === '/login' ? 'login' : 'logout');
        if (cmd === '/clear') {
            setTranscript([]);
